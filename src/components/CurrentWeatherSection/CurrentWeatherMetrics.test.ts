...
vi.mock('@/components/ui/Card', () => ({
  CurrentCard: {
    name: 'CurrentCard',
    template: '<div class="current-card" :data-label="label">{{ value }}{{ unit }}</div>',
...
describe('CurrentWeatherMetrics.vue', () => {
  ...
  it('should render skeleton when loading is true', () => {
    ...
    expect(wrapper.find('.current-weather-section__metrics').exists()).toBe(false);
  });
  ...
  it('should render weather metrics when data is available', () => {
    ...
    expect(wrapper.find('.current-weather-section__metrics').exists()).toBe(true);
